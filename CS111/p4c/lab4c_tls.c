...
void output(char *output_buf) {
	if(SSL_write(tls, output_buf, strlen(output_buf)) < 0) {
		perror("lab4c_tls");
		exit(2);
	}
...
void* command_thread(void *arg __attribute__((unused))) {
	...
	while (1) {
		...
		num_c = SSL_read(tls, &buffer + offset, 1024-offset);
		if (num_c < 0) {
...
int main(int argc, char * argv[]) {
	...
	while ((opt = getopt_long(argc, argv, "", long_options, NULL)) != -1) {
		switch (opt) {
			...
			break;
			default: // '?'
			fprintf(stderr, "Usage: ./lab4b_tls [--period=#] [--scale=C|F] --log=filename --id=xxxxxxxxx --host=hostname portnumber\n");
			exit(EXIT_FAILURE);
			break;
		} // end switch
	} // end while
