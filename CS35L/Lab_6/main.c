 ...
 */
struct ThreadEnv {
    // variables declared in the innermost two for loops
    Vec3 sample_color;
    ray_t pixel_ray;
    // varaibles still in scope outside two for loops
    Vec3 *pixel_color;
    scene_t *scene;
    // const primitive
    int recursionDepth;
...
struct ThreadEnv captureEnv(Vec3 sample_color, ray_t pixel_ray, Vec3 *pixel_color, scene_t *scene, int depth) {
    struct ThreadEnv env;
    // initialize sample_color
    for(int i = 0; i < 3; i++) {
        env.sample_color[i] = sample_color[i];
    }
    // initialize pixel_ray
    for(int i = 0; i < 3; i++) {
        env.pixel_ray.dir[i] = pixel_ray.dir[i];
        env.pixel_ray.org[i] = pixel_ray.org[i];
    }
    // initialize pixel_color
    env.pixel_color = pixel_color;
    // initialize scene
    env.scene = scene;
    // initialize dept
    env.recursionDepth = depth;
    return env;
}

void* traceEntry(void *param) {
    struct ThreadEnv* tep = param;
    trace( tep -> scene, tep -> sample_color, &(tep -> pixel_ray), tep -> recursionDepth );
    add( *(tep -> pixel_color), *(tep -> pixel_color), tep -> sample_color);
    return NULL;
}
...
main( int argc, char **argv )
{
    ...
    struct ThreadEnv envs[nthreads];
    int threadsActive = 0;
        ...
        for( int py=0; py<height; ++py )
        {
            ...
            for( int xs=-halfSamples; xs<=halfSamples; ++xs )
            {
                for( int ys=-halfSamples; ys<=halfSamples; ++ys )
                {
                    ...
                    Vec3 pixel_target; // only used to construct pixel_ray, no need to memorize it.
                    ...
                    if (threadsActive == nthreads) {
                        for (int i = 0; i < nthreads; i++) {
                            pthread_join(threads[i], NULL); // TODO: Error Checking
                            threadsActive--;
                        }
                    }
                    if (threadsActive < nthreads) {
                        envs[threadsActive] = captureEnv(sample_color, pixel_ray, &pixel_color, &scene, 0);
                        int errorCode = 0;
                        if ( (errorCode = pthread_create(&threads[threadsActive], NULL, traceEntry, &envs[threadsActive] )) != 0) {
                            fprintf(stderr, "Error Creating thread with code: %d\n", errorCode);
                        }
                    ...
                    //trace( &scene, sample_color, &pixel_ray, 0 ); // parallelize this
                    /* sum color for subpixel AA */
                    //add( pixel_color, pixel_color, sample_color );
                }
            }
            if (threadsActive != 0) {
                for (int i = 0; i < threadsActive; i++) {
